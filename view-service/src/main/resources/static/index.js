angular.module('market', ['ngStorage']).controller('indexController', function ($scope, $http, $localStorage) {
    ...
    $scope.tryToAuth = function () {
        $http.post('http://localhost:8094/auth/authenticate', $scope.user)
    ...
    $scope.loadProducts = function () {
        $http.get('http://localhost:8094/products/api/v1/products')
    ...
    $scope.loadCart = function () {
        $http.get('http://localhost:8094/cart/api/v1/cart')
            .then(function (response) {
    ...
    $scope.addToCart = function (id) {
        $http.get('http://localhost:8094/cart/api/v1/cart/add/' + id)
            .then(function (response) {
                $scope.loadCart();
            });
    }
